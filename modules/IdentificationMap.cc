...
void IdentificationMap::Process()
{
  ...
  //---------------Get measured value
    ...
    //gas proportion 
    int Opt = 0; 
    if(pdgCodeIn==211||pdgCodeIn==-211||pdgCodeIn==321||pdgCodeIn==-321){
    ...
    //-------------------chi-square
      ...
      chi[1] =  /* (tof_meas - tof_exp)/tof_sigma */0;//(30E-12);//s
      total_chi2 = chi[0]*chi[0] + chi[1]*chi[1];
      Prob[i] = TMath::Prob(total_chi2,1);//积分    
      //test[i] = total_chi2;//test
      candidate->Prob[i] = Prob[i];
      if(i==2){
