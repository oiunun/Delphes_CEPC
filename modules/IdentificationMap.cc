...
void IdentificationMap::Process()
{
    ...
    while((candidate = static_cast<Candidate *>(fItInputArray->Next())))
    {
        ...
        CosTheta = candidateMomentum.CosTheta();
        ...
        //---------------Get measured value
        Double_t p_meas = candidateMomentum.P();
        Double_t dndx_meas = candidate->Nclusters;
        Double_t dndx_sigma = candidate->Nclusters_err;
        ...
        if(pdgCodeIn==211||pdgCodeIn==-211||pdgCodeIn==321||pdgCodeIn==-321||pdgCodeIn==2212||pdgCodeIn==-2212){
            if(dndx_meas!=0 && l>0 && L_DC>0){
                for(int i=2;i<5;i++){

                    Double_t bg = p_meas/mass[i]; 
                    Double_t dndx_exp = TrkUtil::Nclusters(bg,abs(CosTheta)) *L_DC;
