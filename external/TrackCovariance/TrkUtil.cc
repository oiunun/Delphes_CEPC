...
Bool_t TrkUtil::IonClusters(Double_t& Ncl,Double_t& Nclerr, Double_t mass, TVectorD Par)
{
    //
    ...
    Double_t errClu = 0.0;	// sigma number of clusters
    Double_t bg = 0.0;		// beta*gamma
    Double_t cos = 0.0;		// costheta
    Ncl = 0.0;
    if (tLen > 0.0)
    {
        Signal = kTRUE;
        ...
        else
        {
            ...
            bg = p.Mag() / mass;
            cos=abs(p.CosTheta());
            muClu = Nclusters(bg,cos) * tLen;				// Avg. number of clusters
            errClu = NclSigma(bg,cos) * sqrt(tLen);				// Avg. number of clusters
            Ncl = gRandom->Gaus(muClu,errClu);
            Nclerr =errClu;
        }
    }
    //
...
//add full simulation result

Double_t TrkUtil::Nclusters(Double_t begam, Double_t cos) {

    Double_t interp = 0.0;

    const Int_t Npt = 19;
    Double_t bg[Npt]  = { 0.5, 0.8, 1., 2., 3.,4.,5., 8., 10.,12., 15.,20., 50.,75., 100.,150.,200., 500., 10000. };
    double dndx[Npt] = {28.7809,18.3345,15.2892,11,10.3567,10.3151,10.3745,10.7116,10.9019,11.0991,11.3411,11.6233,12.6518,13.0363,13.2492,13.4154,13.5099,13.664,12.6182 } ;


    TSpline3* sp3 = new TSpline3("sp3", bg, dndx, Npt);
    if (begam > bg[0] && begam < bg[Npt - 1]) interp = sp3->Eval(begam)*(1+0.0694315*cos*cos-0.268413*abs(cos*cos*cos));
    delete sp3;
    return 100 * interp;
}
...
Double_t TrkUtil::NclSigma(Double_t begam, Double_t cos) {

    Double_t interp = 0.0;

    const Int_t Npt = 19;
    Double_t bg[Npt]  = { 0.5, 0.8, 1., 2., 3.,4.,5., 8., 10.,12., 15.,20., 50.,75., 100.,150.,200., 500., 10000. };
    double sigma[Npt] = {4.93937,4.41613,4.18186,3.67386,3.57761,3.5599,3.57458,3.60742,3.616,3.59681,3.63773,3.68861,3.75672,3.7808,3.83416,3.84321,3.86808,3.90664,3.77578};


    TSpline3* sp3 = new TSpline3("sp3", bg, sigma, Npt);
    if (begam > bg[0] && begam < bg[Npt - 1]) interp = sp3->Eval(begam)*(1+0.0730019*cos*cos-0.324099*abs(cos*cos*cos));
    delete sp3;
    return 10 * interp * 0.95 ;
}
