#define theta_reso_pt_cxx
#include "theta_reso_pt.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
#include <math.h>
void theta_reso_pt::Loop()
{
...
//   TH2F *sigmaphi = new TH2F("small","#sigma#phi", 100, 0,80,100,0,0.02);
  ...
  for(Int_t i=0;i<26;i++){ char t_hname[20];
    ...
    t_gaus1[i]=new TH1F(t_hname,"",200,-0.02,0.02);}
...
//   double t_error5[26];
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     // cout<<jentry<<" "<<j<<" "<<Track_ErrorD0[j]<<endl;
      Float_t theta_truth = acos(Track_Truth_CosTheta[j]);
      Float_t theta = acos(Track_CosTheta[j]);
      Float_t var=theta-theta_truth;
      //  Float_t cut_var = abs(Track_CosTheta[j]);
       if(Track_PT[j]>1.) {
      //   if(var!=0. && cut_var>=0. && cut_var<=0.2 ) { 
        for ( int jp=0;jp<26;jp++){ if(Track_PT[j]>3.*jp && Track_PT[j]<=3.*jp+3.)
          {t_gaus1[jp]->Fill(var); } }/* } */
  ...
  TH1F *pth1[26];        //each phi bin histogram
...
//   double pt5[26];
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     // cout<<jentry<<" "<<j<<" "<<Track_ErrorD0[j]<<endl;
      ...
      //  Float_t cut_var = abs(Track_CosTheta[j]);
       if(Track_PT[j]>1.) {
      //   /* if(var!=0. && cut_var>=0. && cut_var<=0.2 ) { */ sigmaPT->Fill(Track_PT[j],var);
        for ( int jp=0;jp<26;jp++){ if(Track_PT[j]>3.*jp && Track_PT[j]<=3.*jp+3.)
          {gaus1[jp]->Fill(var); pth1[jp]->Fill(Track_PT[j]);} }/* } */
...
//   g[4]=new TGraphErrors(12,pt5,aver5,errpt5,error5);
  ...
  g[0]->GetYaxis()->SetTitle("#sigma#theta(rad)");
  g[0]->GetXaxis()->SetTitle("Pt(GeV/c)");
  g[0]->GetYaxis()->SetRangeUser(1e-5,5e-4);
  TCanvas *c1=new TCanvas("c1","c1",1300,700);
  c1->SetLogy();
  g[0]->Draw("ALP");
//   for(int i=1;i<5;i++) g[i]->Draw("LP same");
//   le->Draw("same");
  c1->Print("../fig/theta_reso_pt.pdf");
  c1->Print("../fig/theta_reso_pt.png");
  // TCanvas*c2=new TCanvas("c2","c2");
  // sigmaPT->Draw();
  TFile *f=new TFile("../fig/root/his_theta_pt.root","recreate");
  for(int i = 0 ;i<26;i++){
    gaus1[i]->Write();
   //  gaus2[i]->Write();
   //  gaus3[i]->Write();
