#define phi_reso_pt_cxx
#include "phi_reso_pt.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void phi_reso_pt::Loop()
{
...
//   TH2F *sigmaphi = new TH2F("small","#sigma#phi", 100, 0,80,100,0,0.02);
  ...
  for(Int_t i=0;i<26;i++){ char t_hname[20];
    ...
    t_gaus1[i]=new TH1F(t_hname,"",200,0,0.02);}
...
//   g[4]=new TGraphErrors(12,pt5,aver5,errpt5,error5);
  ...
  g[0]->GetYaxis()->SetRangeUser(1e-5,6e-4);
  TCanvas *c1=new TCanvas("c1","c1",1300,700);
  c1->SetLogy();
  g[0]->Draw("ALP");
//   for(int i=1;i<5;i++) g[i]->Draw("LP same");
//   le->Draw("same");
  c1->Print("../fig/Phi_reso_pt.pdf");
  c1->Print("../fig/Phi_reso_pt.png");
  // TCanvas*c2=new TCanvas("c2","c2");
  // sigmaPT->Draw();
  TFile *f=new TFile("../fig/root/his_Phi_pt.root","recreate");
  for(int i = 0 ;i<26;i++){
    gaus1[i]->Write();
   //  gaus2[i]->Write();
   //  gaus3[i]->Write();
