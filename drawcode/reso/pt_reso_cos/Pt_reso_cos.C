#define pt_reso_cos_cxx
#include "pt_reso_cos.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void pt_reso_cos::Loop()
{
...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     // cout<<jentry<<" "<<j<<" "<<Track_ErrorD0[j]<<endl;
       Float_t var=Track_ErrorPT[j]/Track_PT[j];
  ...
  for(Int_t i=0;i<26;i++){ char hname[20];
    ...
    gaus1[i]=new TH1F(hname,"",200,t_aver1[i]-10*t_error1[i],t_aver1[i]+10*t_error1[i]);}
  ...
  for(Int_t i=0;i<26;i++){ char hname[20];
    ...
    if(i==25) gaus2[i]=new TH1F(hname,"",200,t_aver2[i]-10*t_error2[i],t_aver2[i]+20*t_error2[i]);
    else gaus2[i]=new TH1F(hname,"",200,t_aver2[i]-10*t_error2[i],t_aver2[i]+10*t_error2[i]);}
  ...
  TLegend *le=new TLegend(0.4, 0.6, 0.6, 0.8);
  ...
  g[0]->GetYaxis()->SetTitle("#sigmaPT/PT");
  ...
  le->Draw("same");
  c1->Print("../fig/pt_reso_cos.pdf");
  c1->Print("../fig/pt_reso_cos.png");
  // TCanvas*c2=new TCanvas("c2","c2");
  // sigmaPT->Draw();
  TFile *f=new TFile("../fig/root/his_pt_cos.root","recreate");
  for(int i = 0 ;i<26;i++){
    gaus1[i]->Write();
    gaus2[i]->Write();
    gaus3[i]->Write();
