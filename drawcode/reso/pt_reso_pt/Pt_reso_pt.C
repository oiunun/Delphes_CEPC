#define pt_reso_pt_cxx
#include "pt_reso_pt.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void pt_reso_pt::Loop()
{
...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  g[0]->GetYaxis()->SetRangeUser(0.001,0.0035);
  TCanvas *c1=new TCanvas("c1","c1",1300,700);
  // c1->SetLogy();
  g[0]->Draw("ALP");
  for(int i=1;i<5;i++) g[i]->Draw("LP same");
  le->Draw("same");
  c1->Print("../fig/pt_reso_pt.pdf");
  c1->Print("../fig/pt_reso_pt.png");
  // TCanvas*c2=new TCanvas("c2","c2");
  // sigmaPT->Draw();
  TFile *f=new TFile("../fig/root/his_pt_pt.root","recreate");
  for(int i = 0 ;i<26;i++){
    gaus1[i]->Write();
    gaus2[i]->Write();
    gaus3[i]->Write();
