#define d0_reso_pt_cxx
#include "d0_reso_pt.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void d0_reso_pt::Loop()
{
...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  for(Int_t i=0;i<26;i++){ char t_hname[20];
    ...
    t_gaus5[i]=new TH1F(t_hname,"",200,0,0.03);}
  ...
  g[0]->GetYaxis()->SetTitle("#sigmaD0(mm)");
  g[0]->GetXaxis()->SetTitle("Pt(GeV/c)");
  g[0]->GetYaxis()->SetRangeUser(1e-3,2e-2);
  TCanvas *c1=new TCanvas("c1","c1",1300,700);
  c1->SetLogy();
  g[0]->Draw("ALP");
  for(int i=1;i<5;i++) g[i]->Draw("LP same");
  le->Draw("same");
  c1->Print("../fig/d0_reso_pt.pdf");
  c1->Print("../fig/d0_reso_pt.png");
  // TCanvas*c2=new TCanvas("c2","c2");
  // sigmaPT->Draw();
  TFile *f=new TFile("../fig/root/his_d0_pt.root","recreate");
  for(int i = 0 ;i<26;i++){
    gaus1[i]->Write();
    gaus2[i]->Write();
    gaus3[i]->Write();
