...
void FormatRooFit(RooPlot* rooplot, Char_t * xtitle="M(GeV/c^{2})", Char_t * ytitle="Entries", double yoffset=1.2);
void PlotDataRooFit(Char_t * figfilename, 
		RooAbsData* data, RooRealVar x, 
		vector<RooAbsPdf*> pdfList, 
		char* xtitle="M(GeV/c^{2})", 
		char* ytitle="Entries", bool prelim=false,
		char* channel= "ZH, Z#rightarrow #mu^{+}#mu^{-}; #int L=5 ab^{-1}",
		double maxy=-1
		);
//
// Name histogram axes
...
void WriteChannel(char* channel="ZH, Z#rightarrow #mu^{+}#mu^{-}; #int L=5 ab^{-1}"); // to be used together with WriteCEPC()
...
void MakeLegend(TH1  * datahist,         // Histogram with data
		...
		double yhi  = 0.95);               // Top edge of legend    (fraction of canavas height)
// position will have to change depending on the data shape
...
void PlotDataFit(
		...
		double yhi  = 0.7,
		char* channel= "ZH, Z#rightarrow #mu^{+}#mu^{-}; #int L=5 ab^{-1}"
		);                // Top edge of legend    (fraction of canavas height)
