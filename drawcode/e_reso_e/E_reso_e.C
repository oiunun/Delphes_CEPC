...
//   double t_error5[30];
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     // cout<<jentry<<" "<<j<<" "<<Track_ErrorD0[j]<<endl;
      Float_t var = (Photon_Truth_E[j]-Photon_E[j])/Photon_Truth_E[j];
      //  Float_t cut_var = abs(Track_CosTheta[j]);
       if(Photon_Truth_E[j]>1.) {
      //   if(var!=0. && cut_var>=0. && cut_var<=0.2 ) { 
        for ( int jp=0;jp<30;jp++){ if(Photon_Truth_E[j]>3.*jp && Photon_Truth_E[j]<=3.*jp+3.)
          {t_gaus1[jp]->Fill(var); } }/* } */
...
//   double pt5[30];
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     Float_t var = (Photon_Truth_E[j]-Photon_E[j])/Photon_Truth_E[j];
      //  Float_t cut_var = abs(Track_CosTheta[j]);
       if(Photon_Truth_E[j]>1.) {
      //   /* if(var!=0. && cut_var>=0. && cut_var<=0.2 ) { */ sigmaPT->Fill(Photon_Truth_E[j],var);
        for ( int jp=0;jp<30;jp++){ if(Photon_Truth_E[j]>3.*jp && Photon_Truth_E[j]<=3.*jp+3.)
          {gaus1[jp]->Fill(var); pth1[jp]->Fill(Photon_Truth_E[j]);} }/* } */
...
//   g[4]=new TGraphErrors(12,pt5,aver5,errpt5,error5);
  ...
  g[0]->GetYaxis()->SetTitle("#sigmaE/E");
  g[0]->GetXaxis()->SetTitle("E(GeV)");
  g[0]->GetYaxis()->SetRangeUser(6e-3,3e-2);
  TCanvas *c1=new TCanvas("c1","c1",1300,700);
  c1->SetLogy();
  g[0]->Draw("ALP");
//   for(int i=1;i<5;i++) g[i]->Draw("LP same");
//   le->Draw("same");
  c1->Print("../fig/E_reso_e.pdf");
  c1->Print("../fig/E_reso_e.png");
  // TCanvas*c2=new TCanvas("c2","c2");
  // sigmaPT->Draw();
  TFile *f=new TFile("../fig/root/his_E_E.root","recreate");
  for(int i = 0 ;i<30;i++){
    gaus1[i]->Write();
   //  gaus2[i]->Write();
   //  gaus3[i]->Write();
