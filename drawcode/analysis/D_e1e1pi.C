...
void Com_BvD(ExRootTreeReader *treeReader,ExRootTreeReader *treeReaderB);
...
void D_e1e1pi()
{
  ...
  TChain chainB("Delphes");
  chain.Add("../../rootfile/D_e1e1pi.root");
  chainB.Add("../../rootfile/B_e1e1pi.root");
  ...
  ExRootTreeReader *treeReaderB = new ExRootTreeReader(&chainB);
  ...
  // Test(treeReader);
  Com_BvD(treeReader,treeReaderB);
...
void Eem_Ehad(ExRootTreeReader *treeReader){
  ...
  TFile *f=new TFile("fig/D_e1e1pi/Edepo_reso.root","recreate");
  for(int i=0;i<20;i++){
    hisEem_reso[i]->Write();
  ...
  // c1->cd(15);
  // histEdepo_p->Draw();
  c1->Print("fig/D_e1e1pi/Eem_Ehad.pdf"); 
  c1->Print("fig/D_e1e1pi/Eem_Ehad.png"); 
  delete c1;
  ...
  c2->cd(2);
  Ehad_reso->Draw("APL");
  c2->Print("fig/D_e1e1pi/Edepo_reso.png");
  c2->Print("fig/D_e1e1pi/Edepo_reso.pdf");
}
...
void Signal_Mass(ExRootTreeReader *treeReader){
    ...
    hist_sig_mass->Draw();
    c1->Print("fig/D_e1e1pi/hist_mass.png"); 
    c1->Print("fig/D_e1e1pi/hist_mass.pdf"); 
    c1->Clear();
    delete c1;
}
...
void Sig_Bg_Mass(ExRootTreeReader *treeReader,ExRootTreeReader *treeReaderbg){
    ...
    c1->Print("fig/D_e1e1pi/hmass_sig_bg.png"); 
    c1->Print("fig/D_e1e1pi/hmass_sig_bg.pdf"); 
    c1->Clear();
    delete c1;
}
...
void Truth_Mass(ExRootTreeReader *treeReader){
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry){
    ...
    for(Long64_t i=0;i<branchParticleFlowCandidate->GetEntriesFast();i++){
      ...
      particleflowcandidate= (ParticleFlowCandidate*) branchParticleFlowCandidate->At(i);
      particle = (GenParticle*) particleflowcandidate->Particles.At(0);
      for(it_D = id_D.begin();it_D!=id_D.end();it_D++){
        if(particle->M1==*it_D && particleflowcandidate->truth_PID != 130){
          switch(particle->PID){
            case 211  : 
              id_pi.insert(pair<Int_t,Int_t>(distance(id_D.begin(),it_D),i));
              break;
            case -211  : 
              id_pi.insert(pair<Int_t,Int_t>(distance(id_D.begin(),it_D),i));
              break;
            case 11 :
              id_ep.insert(pair<Int_t,Int_t>(distance(id_D.begin(),it_D),i));
              break;
            case -11 :
              id_em.insert(pair<Int_t,Int_t>(distance(id_D.begin(),it_D),i));
              break;
          }
        }
      }
    ...
    // for(it_D = id_D.begin();it_D!=id_D.end();it_D++){
    //   for(it_pi=id_pi.begin();it_pi!=id_pi.end();it_pi++){
    //     if(it_pi->first ==distance(id_D.begin(),it_D)){}
    //   }
    // }
  ...
  c1->Print("fig/D_e1e1pi/hist_truth_mass.png"); 
  c1->Print("fig/D_e1e1pi/hist_truth_mass.pdf"); 
  c1->Clear();
  delete c1;
}
void TrackEfficiency(ExRootTreeReader *treeReader){
  ...
  // c1->Print("fig/D_e1e1pi/TrackEfficiency.png"); 
  // c1->Print("fig/D_e1e1pi/TrackEfficiency.pdf"); 
  c1->cd(2);
  hfrac->Draw();
  c1->Print("fig/D_e1e1pi/TrackEfficiency.png"); 
...
void Performance_PID(ExRootTreeReader *treeReader){
  ...
  // c1->SetLogx();
  c1->Print("fig/D_e1e1pi/Prob_Pi.png"); 
  c1->Print("fig/D_e1e1pi/Prob_Pi.pdf"); 
  ...
  gPad->BuildLegend(0.65,0.83,0.95,0.95);
  c1->Print("fig/D_e1e1pi/Prob_K.png"); 
  c1->Print("fig/D_e1e1pi/Prob_K.pdf"); 
  delete c1;
...
void Test(ExRootTreeReader *treeReader){
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry)
  {
    ...
    for(Long64_t i=0;i<branchParticle->GetEntriesFast();i++){
      particle= (GenParticle*) branchParticle->At(i);
      for(it_D = id_D.begin();it_D!=id_D.end();it_D++){
        if(abs(particle->PID)==211 && particle->M1 == *it_D){
          hmom_fBpi->Fill(particle->P4().CosTheta());
        }
        if(abs(particle->PID)==11 && particle->M1 == *it_D){
  ...
  c1->cd(2);
  hmom_fBpi->Draw();
  c1->Print("fig/D_e1e1pi/hmom_pi.png"); 
  c1->cd(1);
  hmom_e->Draw();
  c1->cd(2);
  hmom_fBe->Draw();
  c1->Print("fig/D_e1e1pi/hmom_e.png"); 
...
void Com_BvD(ExRootTreeReader *treeReader,ExRootTreeReader *treeReaderB){
  ...
  ParticleFlowCandidate *particleflowcandidate;
  GenParticle *particle;
  TH2F *hmom_fBpi = new TH2F("P:Cos for pi from B", "P:Cos for pi from B", 100,0,30,100,-1,1);
  TH2F *hmom_fBe = new TH2F("P:Cos for e from B", "P:Cos for e from B", 100,0,30,100,-1,1);
  TH2F *hmom_fDpi = new TH2F("P:Cos for pi from D", "P:Cos for pi from D", 100,0,30,100,-1,1);
  TH2F *hmom_fDe = new TH2F("P:Cos for e from D", "P:Cos for e from D", 100,0,30,100,-1,1);
  TH1F *hmom_B = new TH1F("P for e from B", "P for e from B", 100,0,30);
  TH1F *hmom_D = new TH1F("P for e from D", "P for e from D", 100,0,30);
  TH1F *hcos_D = new TH1F("Cos for e from D", "Cos for e from D", 100,-1,1);
  TH1F *hcos_B = new TH1F("Cos for e from B", "Cos for e from B", 100,-1,1);
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry)
  {
    ...
    for(Long64_t i=0;i<branchParticle->GetEntriesFast();i++){
      ...
      for(it_D = id_D.begin();it_D!=id_D.end();it_D++){
        if(abs(particle->PID)==211 && particle->M1 == *it_D){
          hmom_fDpi->Fill(particle->P,particle->P4().CosTheta());
        }
        if(abs(particle->PID)==11 && particle->M1 == *it_D){
          hmom_fDe->Fill(particle->P,particle->P4().CosTheta());
        }
        if(particle->M1 == *it_D){
          hmom_D->Fill(particle->P);
          hcos_D->Fill(particle->P4().CosTheta());
        }
      }
    }
    
  }
  branchParticle = treeReaderB->UseBranch("Particle");
  branchParticleFlowCandidate = treeReaderB->UseBranch("ParticleFlowCandidate");

  numberOfEntries = treeReaderB->GetEntries();
  for(Int_t entry = 0; entry < numberOfEntries; ++entry)
  {
    // Load selected branches with data from specified event
    treeReaderB->ReadEntry(entry);
    ...
    for(Long64_t i=0;i<branchParticle->GetEntriesFast();i++){
      ...
      for(it_B = id_B.begin();it_B!=id_B.end();it_B++){
        if(abs(particle->PID)==211 && particle->M1 == *it_B){
          hmom_fBpi->Fill(particle->P,particle->P4().CosTheta());
        }
        if(abs(particle->PID)==11 && particle->M1 == *it_B){
          hmom_fBe->Fill(particle->P,particle->P4().CosTheta());
        }
        if(particle->M1 == *it_B){
          hmom_B->Fill(particle->P);
          hcos_B->Fill(particle->P4().CosTheta());
        }
      }
  ...
  hcos_B->SetLineColor(kRed);
  hmom_B->SetLineColor(kRed);
  c1->Divide(2,1);
  c1->cd(1);
  hmom_D->Draw();
  hmom_B->DrawNormalized("same",hmom_D->Integral());
    gPad->BuildLegend(0.65,0.75,0.95,0.95);
  c1->cd(2);
  hcos_D->Draw();
  hcos_B->DrawNormalized("same",hcos_D->Integral());
    gPad->BuildLegend(0.65,0.75,0.95,0.95);
  c1->Print("fig/D_e1e1pi/BvD1F.png"); 
