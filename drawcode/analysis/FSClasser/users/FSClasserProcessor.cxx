...
void FSClasserProcessor::init() { 
	...
	if( m_savehis ){
		...
		delete ntFullName;
		//
		hValid[ 0] = new TH1D("Valid_00", "Total Energy",100,   0.0, 400.0);
		...
		hValid[ 6] = new TH1D("Valid_06", "Vr  of Track",100, -20.0,  20.0); 
		hValid[ 7] = new TH1D("Valid_07", "Vz  of Track",100, -20.0,  20.0); 
		hValid[ 8] = new TH1D("Valid_08", "# of Tracks ",100,  -1.0, 199.0);
		hValid[ 9] = new TH1D("Valid_09", "# of PFOs   ",100,  -1.0, 199.0);
		...
		hValid[15] = new TH1D("Valid_15", "Inv. Mass ll",100,  60.0, 120.0);
		hValid[16] = new TH1D("Valid_16", "# of hits   ",100,   0.0, 100.0);
		...
		//
		//
		hPando[ 0] = new TH1D("Pando_00", "Total Energy",100,   0.0, 400.0);
		...
		hPando[ 6] = new TH1D("Pando_06", "Vr  of Track",100, -20.0,  20.0); 
		hPando[ 7] = new TH1D("Pando_07", "Vz  of Track",100, -20.0,  20.0); 
		hPando[ 8] = new TH1D("Pando_08", "# of Tracks ",100,  -1.0, 199.0);
		hPando[ 9] = new TH1D("Pando_09", "# of PFOs   ",100,  -1.0, 199.0);
		...
		hPando[15] = new TH1D("Pando_15", "Inv. Mass ll",100,  60.0, 120.0);
		hPando[16] = new TH1D("Pando_16", "# of hits   ",100,   0.0, 100.0);
...
void FSClasserProcessor::processEvent( LCEvent * evt ) { 
	...
	_nEvt ++ ;
	if( m_debug>1 ) printf("\n");
	if( m_debug>1 || _nEvt%1000==0 ) printf("Run & event =  %8d %8d\n",_nRun, _nEvt);
	//
	...
	//LCCollection    *colPFOArborC   = NULL; 
	//LCCollection    *colPFOArborN   = NULL; 
	if( m_full>0 && m_makeplots>0 )  colPFOPandora = evt->getCollection( "PandoraPFOs" );
	//	
	if( _colPFOs != "" ) colPFOs = evt->getCollection( _colPFOs );
	/*
	else{
		colPFOArborC  = evt->getCollection( "ArborCharged" );
		colPFOArborN  = evt->getCollection( "ArborNeutral" );
		LCCollectionVec *NewPFOsCollection  = new LCCollectionVec(LCIO::RECONSTRUCTEDPARTICLE);
		NewPFOsCollection->setSubset(true);
		//
		for (Int_t i=0; i<colPFOArborC->getNumberOfElements(); i++) {
			ReconstructedParticle *recPart = dynamic_cast<ReconstructedParticle*>(colPFOArborC->getElementAt(i));
			//TrackVec   tckvec = recPart->getTracks();
			NewPFOsCollection->addElement(recPart);
		}
		//
		for (Int_t i=0; i<colPFOArborN->getNumberOfElements(); i++) {
			ReconstructedParticle *recPart = dynamic_cast<ReconstructedParticle*>(colPFOArborN->getElementAt(i));
			NewPFOsCollection->addElement(recPart);
		}
		evt->addCollection(NewPFOsCollection,    "ArborPFOs");
		colPFOs = evt->getCollection( "ArborPFOs" );
		//cout<<"No of PFO = "<<NewPFOsCollection->size()<<endl;
		//delete colPFOArborC;
		//delete colPFOArborN;
	}
	*/
	if( m_findJets &&  _colJets != ""){
	       	colJets  = evt->getCollection(_colJets);
	}
	if( (m_findTaup ||  m_findTaum) && _colTaus != ""  ){
	       	colTaus  = evt->getCollection(_colTaus);
	}
	//if( m_findEp ||  m_findEm || m_findMup || m_findMum || _colIsoLeps != "") 
	if( _colIsoLeps != "") 
	{
	...
	// 7: ee; 8: mumu; 9 tautau; 10: gammagamma;
	// 11: gluon, gluon, 12: gamma Z; 13: ZZ, 14: WW
	_nhfs = m_mcTruthHelper->nHiggsFinalState( );
	//
        int N_v =0; int N_e =0; int N_mu =0; int N_etau = 0; int N_mutau = 0; int N_htau =0; int N_b =0; int N_c =0; int N_s =0;
        int N_u =0; int N_d =0; int N_g =0; int N_p = 0;	

        int NN_b=0; int NN_c=0; int NN_s=0; int NN_u=0; int NN_d=0; int NN_g =0;
        TString qqH_FS(string(qqH(m_mcTruthHelper,N_v,N_e,N_mu, N_etau,N_mutau, N_htau,N_b, N_c, N_s,N_u,N_d,N_g,N_p)));      

 //       std::cout<<"qqH_FS = "<<qqH_FS<<std::endl;
        
        cout<<"================Event : "<<_nEvt<<"==================="<<endl;
        vector<MCParticle*> partonlist;
        partonlist.clear();
	BuildFullSimParticleList( partonlist, colMC, colJets, colMCTL,   colPFOs, colPFOPandora, colIsoLeps, colTaus);
        int nparton = 0;
        
          for(size_t id=0;id<partonlist.size();id++){
//               cout<<"Parton4Match["<<id<<"]"<<" : "<<partonlist.at(id)->getPDG()<<", Energy = "<<partonlist.at(id)->getEnergy()<<", (px,py,pz) = ("<<partonlist.at(id)->getMomentum()[0]<<", "<<partonlist.at(id)->getMomentum()[1]<<", "<<partonlist.at(id)->getMomentum()[2]<<")"<<endl;
              TLorentzVector p_parton;
              int parton_pdg = partonlist.at(id)->getPDG();
              p_parton.SetPxPyPzE(partonlist.at(id)->getMomentum()[0],partonlist.at(id)->getMomentum()[1],partonlist.at(id)->getMomentum()[2],partonlist.at(id)->getEnergy());
              if(fabs(p_parton.Eta())<2.44 && ((p_parton.E()>10 && abs(parton_pdg)<6)||(p_parton.E()>15 && abs(parton_pdg)==21))){
                  nparton +=1;
                  if(parton_pdg == 21) NN_g += 1;
                  else if(abs(parton_pdg) == 5)NN_b += 1;
                  else if(abs(parton_pdg) == 4)NN_c += 1;
                  else if(abs(parton_pdg) == 3)NN_s += 1;
                  else if(abs(parton_pdg) == 2)NN_u += 1;
                  else if(abs(parton_pdg) == 1)NN_d += 1;
              }
          }
	...
	/*
	   ...
	   */
	if(m_debug>1){
		if (m_findJets   ) printf("No. of jets       = %3d\n",  (int)    jetList.size()) ;
		...
		for (unsigned int i = 0; i < pCombos.size(); i++){
			...
			if (exclusive ){
				if( fsinfo->missingMassFit() ){
					if(fabs(missingMass-fsinfo->missingMassValue())> 1000000)continue;
				}	
				else if (fabs(missingEnergy)                      >10000000)continue;
			}
			m_cutpass[ifs][5]++;
			//
			if (exclusive && !fsinfo->missingMassFit() ){
				double missingP = (m_ecms-totalP4).Rho();	
				if (fabs(missingP)>10000000) continue;
			}
			m_cutpass[ifs][6]++;
			//
			if (exclusive){
				if ( fabs(missingMass2) > 10000000) {
                                    continue;
                                } 
			}
			if (inclusive){
			...
			NT->fillDouble("N_e",N_e);
                        NT->fillDouble("N_mu",N_mu);
                        NT->fillDouble("N_v",N_v);
                        NT->fillDouble("N_etau",N_etau);
                        NT->fillDouble("N_mutau",N_mutau);  
			NT->fillDouble("N_htau",N_htau);
                        NT->fillDouble("N_b",N_b);
                        NT->fillDouble("N_c",N_c);
                        NT->fillDouble("N_s",N_s);
                        NT->fillDouble("N_u",N_u);
                        NT->fillDouble("N_d",N_d);
                        NT->fillDouble("N_g",N_g);
                        NT->fillDouble("N_p",N_p);
                        NT->fillDouble("N_parton", nparton);
                        NT->fillDouble("NN_g",NN_g);
                        NT->fillDouble("NN_b",NN_b);
                        NT->fillDouble("NN_c",NN_c);
                        NT->fillDouble("NN_s",NN_s);
                        NT->fillDouble("NN_d",NN_d);
                        NT->fillDouble("NN_u",NN_u);
			if(m_savemc)NT->fillMCTruth(m_mcTruthHelper);
			if(m_luxury && m_full>0){
				NT->fillTracks(raw_trackvec);
			}
			if(m_luxury){
					NT->fillPFOs(raw_PFOs);
			}
			//
			...
			for (unsigned int j = 0; j < mupList.size(); j++){
				int m=0;
                                cout<<"Filling Mu+ : combo size = "<<combo.size()<<endl;
				for (unsigned int mn = 0; mn < combo.size(); mn++){
					FSParticle* fsp = combo[mn];
					if ( fsp == mupList[j] ) m++;
				}
			...
			for (unsigned int j = 0; j < combo.size(); j++){
				...
				if( fsp->type()!=4)
					...
					NT->fillJet(fsp            , j+1,"Jet", m_full , m_kappa);
			}
			//
...
//                                cout<<"Muon PFO size = "<<muonPFOs.size()<<endl;
			...
			if( rawp4list.size()>=4 ){
				for(unsigned int kl=0; kl<rawp4list.size()-3; kl++){
					for(unsigned int ki=kl+1; ki<rawp4list.size()-2; ki++){
                        ...
                        int qqH_FinalState = -1;

                        if(qqH_FS.Contains("Hbb")||qqH_FS.Contains("Hgg")||qqH_FS.Contains("Hcc")||qqH_FS.Contains("Hss")||qqH_FS.Contains("Hdd")||qqH_FS.Contains("Huu")){
                                    if(qqH_FS.Contains("Hgg"))
                                      qqH_FinalState = 0;
                                    else if(qqH_FS.Contains("Hbb"))
                                      qqH_FinalState = 5;
                                    else if(qqH_FS.Contains("Hcc"))
                                      qqH_FinalState = 4;
                                    else if(qqH_FS.Contains("Hss")) 
                                      qqH_FinalState = 3;
                                    else if(qqH_FS.Contains("Huu"))  
                                      qqH_FinalState = 2;
                                    else if(qqH_FS.Contains("Hdd"))  
                                      qqH_FinalState = 1; 
                        }

//                        cout<<"qqHFS = "<<qqH_FinalState<<endl;
                        NT->fillDouble("qqHFS",qqH_FinalState);
...
//
void    
FSClasserProcessor::BuildFullSimParticleList( vector<MCParticle*> & partonlist,
		LCCollection    *colMC  , LCCollection    *colJets , 
	...
	vector<MCParticleImpl*>  partonList = m_mcTruthHelper->partonList();
        vector<MCParticle*>      partonlist4match = MakePartonList(m_mcTruthHelper);
/*
        for(size_t id=0;id<partonList.size();id++){
             cout<<"Parton["<<id<<"]"<<" : "<<partonList.at(id)->getPDG()<<", Energy = "<<partonList.at(id)->getEnergy()<<", (px,py,pz) = ("<<partonList.at(id)->getMomentum()[0]<<", "<<partonList.at(id)->getMomentum()[1]<<", "<<partonList.at(id)->getMomentum()[2]<<")"<<endl;             
        }
*/
/*         
        int nparton = 0;
        if(partonlist4match.size()>=4){
          for(size_t id=0;id<partonlist4match.size();id++){
               cout<<"Parton4Match["<<id<<"]"<<" : "<<partonlist4match.at(id)->getPDG()<<", Energy = "<<partonlist4match.at(id)->getEnergy()<<", (px,py,pz) = ("<<partonlist4match.at(id)->getMomentum()[0]<<", "<<partonlist4match.at(id)->getMomentum()[1]<<", "<<partonlist4match.at(id)->getMomentum()[2]<<")"<<endl;
              TLorentzVector p_parton;
              int parton_pdg = <partonlist4match.at(id)->getPDG();
              p_parton.SetPxPyPzE(partonlist4match.at(id)->getMomentum()[0],partonlist4match.at(id)->getMomentum()[1],partonlist4match.at(id)->getMomentum()[2],partonlist4match.at(id)->getEnergy());
              if(fabs(p_parton.Eta())<2.44 && ((p_parton.E()>10 && abs(parton_pdg)<6)||(p_parton.E()>15 && abs(parton_pdg)==21)))p_parton +=1;
          }
        }
*/
        partonlist = partonlist4match;  

	//
	m_Sphericity = -1, m_Aplanarity = -1, m_C = -1, m_D = -1;  
	ntrks         = 0;
	nclus         = 0;
	nPFOs         = 0;
	ntrks_Pandora = 0;
	...
	if( colIsoLeps ){
		...
		for (int i=0;i<nLeps;i++) {
			...
			if ( energy>m_EnergyCut  && energy<m_ECM*0.6 ) {
				if( m_full>0 ){	
					int islepton = abs(recPart->getType());//IsLepton(recPart);
					if(m_debug>3)	printf("Lepton ID is %2d\n", islepton);
	...
	if( colJets ){
		...
		if ( numberJets > 0) {
			...
			for (int j=0 ; j < numberJets ; j++ ) {
				...
				jetList .push_back(new FSParticle(pJet, colMCTL, partonlist4match,  "jet", btag, ctag, bctag, cat, flavor));
...
double FSClasserProcessor::Lsphericity() {
	...
	if( m_Sphericity<0 && raw_PFOs.size()>1){
		...
		for (unsigned int i = 0; i<raw_PFOs.size(); i++) {
			ReconstructedParticle* v = raw_PFOs[i];
			TLorentzVector trkVec(v->getMomentum()[0],v->getMomentum()[1],v->getMomentum()[2],v->getEnergy());
			double pMag = TVector3(v->getMomentum() ).Mag();
			norm += pMag;
...
vector<MCParticle *> FSClasserProcessor::MakePartonList(MCTruthHelper* truthhelper){
        ...
        for(unsigned int idx=0; idx<mc_list.size(); idx++){
             if(mc_list.at(idx)->getPDG() == 92){
                 for(unsigned int j = 0; j< mc_list.at(idx)->getParents().size();j++){
                        mc_before_hadronization->push_back(mc_list.at(idx)->getParents().at(j));
                 }
             }  //EndOf Loop in 92's parents
        }//Endof Loop of mc particle

        return *mc_before_hadronization;
}
*/
