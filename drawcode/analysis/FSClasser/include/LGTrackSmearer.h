#ifndef LGTrackSmearer_h
#define LGTrackSmearer_h 1
...
namespace marlin{
	 ...
	 *
	 *  @author F. Gaede, DESY
	 *  @version $Id: LGTrackSmearer.h,v 1.2 2005-10-11 12:56:28 gaede Exp $ 
	 */
	struct TrackResolution {
		...
		TrackResolution(float dPP, float dPT, float bF, float thMin, float thMax) :
			DPP(dPP) ,
			DPT(dPT) ,
			BF(bF) ,
			ThMin(thMin) ,
	...
	class LGTrackSmearer : public IFourVectorSmearer {

		typedef std::vector<TrackResolution> ResVec ;

		public:

		LGTrackSmearer(const std::vector<float>& resVec ) ;


		/** Virtual d'tor.*/
		virtual ~LGTrackSmearer() {} 
