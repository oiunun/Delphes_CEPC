...
void draw_invar_mass(const char * inputFile)
{
  ...
  // TClonesArray *branchPhoton = treeReader->UseBranch("Photonpair");
  // TClonesArray *branchJet = treeReader->UseBranch("Jet");

  TLorentzVector cms(0,0,0,240);
  // Book histograms
  TH1 *histMass_muon = new TH1F("mass", "M_{inv}(#mu_{1}, #mu_{2})", 100, 75, 105.);
  histMass_muon->GetYaxis()->SetTitle("Entries/0.3GeV");
  histMass_muon->GetXaxis()->SetTitle("M^{#mu^{+}#mu^{-}}[GeV]");
  TH1 *histMass_recoil = new TH1F("mass_rec", "M_{recoil}", 100, 120, 140.);
  TH1 *histMass_rr = new TH1F("mass_rr" , "M_{inv}(r_{1}, r_{2})", 100, 120., 130.);
  histMass_rr->GetYaxis()->SetTitle("Entries/0.1GeV");
  histMass_rr->GetXaxis()->SetTitle("M^{#gamma#gamma}[GeV]");
  TH1 *histMass_jet = new TH1F("mass_jet" , "M_{inv}(jet_{1}, jet_{2})", 100, 70., 110.);
  histMass_jet->GetYaxis()->SetTitle("Entries/0.4GeV");
  histMass_jet->GetXaxis()->SetTitle("M^{jet1,jet2}[GeV]");
// Int_t total_nomatch = 0,total_excessmatch = 0;
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry)
  {
    ...
    for(Int_t i=0;i<branchParticleFlowCandidate->GetEntriesFast();++i){

      ParticleFlowCandidate *particleflowcandidate= (ParticleFlowCandidate *) branchParticleFlowCandidate->At(0);
      particle = (GenParticle*) particleflowcandidate->Particles.At(0);

      Float_t Eem =particleflowcandidate->Eem;
    }
  ...
  //  }
    ...
    // c1->Print("fig/histMass_rr.pdf"); 
    // c1->Print("fig/histMass_rr.png"); 
