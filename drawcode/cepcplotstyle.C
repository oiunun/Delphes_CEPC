...
void FormatHist(TH1 * datahist , int n){
   ...
   if( datahist->GetXaxis())  FormatAxis(datahist->GetXaxis());
   if( datahist->GetYaxis())  FormatAxis(datahist->GetYaxis(), 1.3);
}
...
void NameAxes(TH1 * datahist, char * xname, char * yname){
   if(xname) datahist->GetXaxis()->SetTitle(xname);
   if(yname) datahist->GetYaxis()->SetTitle(yname);
}
...
{ 
	...
	if(strstr(datahist->GetTitle(),"CUT") ){
		...
		if( ipos>3) sname = tname.Remove(ipos-2,tname.Sizeof());
		if( ipos<3) sname = tname.Remove(ipos,tname.Sizeof());
		//cout<<ipos<< " "<<tname.Remove(ipos,tname.Sizeof())<<" "<<scut<<endl;
		//cout<<ipos<< " "<<sname<<endl;
		datahist->SetTitle(sname);
		if(mc1hist)mc1hist->SetTitle(sname);
		if(mc2hist)mc2hist->SetTitle(sname);
		if(mc3hist)mc3hist->SetTitle(sname);
		if(mc4hist)mc4hist->SetTitle(sname);
	}
	//
	TCanvas * c1 = new TCanvas("CEPCplots","CEPC Plots", 800,800);
	FormatData(datahist);
	if(mc1hist)FormatMC1(mc1hist);
	if(mc2hist)FormatMC2(mc2hist);
	if(mc3hist)FormatMC3(mc3hist);
	if(mc4hist)FormatMC4(mc4hist);
	//
	if(mc1hist)datahist->SetMaximum(max(datahist->GetMaximum(),mc1hist->GetMaximum())*h_scale);
	if(mc2hist)datahist->SetMaximum(max(datahist->GetMaximum(),mc2hist->GetMaximum())*h_scale);
	if(mc3hist)datahist->SetMaximum(max(datahist->GetMaximum(),mc3hist->GetMaximum())*h_scale);
	if(mc4hist)datahist->SetMaximum(max(datahist->GetMaximum(),mc4hist->GetMaximum())*h_scale);
	//
	if(logy){
		gPad->SetLogy();	
...
void PlotDataFit(char * filename,        // Name for the output files, 
	...
	int linestyles[] = {1,1,1,10,9,7,6,5,};
	...
	for(unsigned int i =0;  i < nfun; i++){
		...
		//cout<<i<<" "<<title<<endl;
		if (title.Contains("special TPaveText")) continue;
...
void PlotDataRooFit(Char_t * figfilename, 
	...
	Double_t yoffset = 1.1+0.13*log(num)/log(10.);
	// 
	RooPlot* xframe = x.frame(100);
	FormatRooFit(xframe, xtitle, ytitle, yoffset);  
	if( maxy >0) xframe->SetMaximum(maxy);
	//
	...
	//
	TLegend * leg = new TLegend( 0.7, 0.68, 0.9, 0.80);
	Char_t name[100];	
	if(data)
	...
	for( unsigned int i =0; i<pdfList.size(); i++){
		...
		if(i==0) leg->AddEntry(line, name, "LF");
		else     leg->AddEntry(line, name, "LF");
	}
